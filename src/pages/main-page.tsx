...
import { getRandomItem, runToHoursAndMin } from '../utils/utils';
...
import { useFetchMoviesRandomQuery } from '../services/movie-api';
...
function MainPage({ ...props }: MainPageProps) {
  ...
  const [
    postFavorites,
    { data: favoritesId, isSuccess: favoritesIdIsSuccess }
  ] = usePostFavoritesMutation();

  const {
    data: dataMoviesRandomQuery,
    error,
    status,
    isSuccess,
    refetch
  ...
  const handleClickToFilmPage = () => {
    if (isSuccess) {
      const href = `/genres/${dataMoviesRandomQuery.genres[0]}/film/${dataMoviesRandomQuery.id}`;
      navigate(href);
    }
  };
  ...
  return (
    <SamplePage {...props}>
      ...
      {status === QueryStatus.fulfilled && dataMoviesRandomQuery && (
        <PageMain className='page-main'>
          ...
          <div className='page-main__wrapper wrapper'>
            <Trailer
              ...
              url={`${dataMoviesRandomQuery.trailerUrl}autoplay=1&mute=1`}
              posterUrl={dataMoviesRandomQuery.posterUrl}
              title={dataMoviesRandomQuery.title}
              ...
              <FilmWrapper className='film-random__wrapper'>
                    ...
                    <List className='film-random__list list list-reset'>
                      <Rating className='list__item'>
                        ...
                        <RatingValue className='list__item-value'>
                          {Math.ceil(dataMoviesRandomQuery.tmdbRating * 100) /
                            100}
                        </RatingValue>
                      </Rating>

                      <li className='list__item'>
                        {dataMoviesRandomQuery.relaseYear}
                      </li>
                      <li className='list__item'>
                        {getRandomItem(dataMoviesRandomQuery.genres)}
                      </li>
                      <li className='list__item'>
                        {runToHoursAndMin(dataMoviesRandomQuery.runtime)}
                      </li>
                    </List>
                  </div>
                  <Title className='film-random__title'>
                    {dataMoviesRandomQuery.title}
                  </Title>
                  <Description className='film-random__description'>
                    {dataMoviesRandomQuery.plot}
                  </Description>
                    ...
                    <ButtonIcon
                      ...
                      onClick={() =>
                        handleAddToFavorites(
                          dataMoviesRandomQuery.id.toString()
                        )
                      }
                    >
                      <Icon
                ...
                <div className='film-random__image random-picture'>
                  <img
                    ...
                    src={dataMoviesRandomQuery.posterUrl}
                    width='552px'
                    height='680px'
                  />
                </div>
